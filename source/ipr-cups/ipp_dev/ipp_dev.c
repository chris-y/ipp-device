/*
 * ipp_dev.c (c) Chris Young 2013
 * Somewhat based on lpr.device by Olaf Barthel
 * and utilising a modified ipr by ShineSoft Systems
 */
...
int ipp_print_file(const char *destUrl, const char *filename, const char *tmplpath);

/****************************************************************************/

#include "ipp.device_rev.h"
...
UnitProcessEntry(VOID)
{
	...
	LONG file_size = 0;
	BOOL ready = FALSE;
	UBYTE temp_buffer[2048];
	UBYTE control_buffer[1024];
	char printer[1024];
	char path[1024];
	char template[1024];
	char *uri;
				...
				case CMD_OpenUnit:
					{
						...
						SPrintf(db,env_name,"%s/%ld",thisProcess->pr_Task.tc_Node.ln_Name,du->du_Index);

						got_env = FALSE;
						if(GetVar(env_name,temp_buffer,sizeof(temp_buffer)-1,GVF_GLOBAL_ONLY) > 0)
						{
							got_env = TRUE;
						}
						else
						{
							strcpy(env_name,thisProcess->pr_Task.tc_Node.ln_Name);
							if(GetVar(env_name,temp_buffer,sizeof(temp_buffer)-1,0) > 0)
								got_env = TRUE;
						}
						...
						if(got_env)
						{
							...
							rda = (struct RDArgs *)AllocDosObjectTagList(DOS_RDARGS,NULL);
							if(rda != NULL)
							{
								strcat(temp_buffer,"\n");

								rda->RDA_Source.CS_Buffer = temp_buffer;
								rda->RDA_Source.CS_Length = strlen(temp_buffer);
								rda->RDA_Source.CS_CurChr = 0;
								rda->RDA_Flags |= RDAF_NOPROMPT;

								memset(&cmd_args,0,sizeof(cmd_args));

								if(ReadArgs(cmd_template,(LONG *)&cmd_args,rda))
								{
									if(cmd_args.Printer) {
										strncpy(printer,cmd_args.Printer,sizeof(printer)-1);
										printer[sizeof(printer)-1] = '\0';
									}

									if(cmd_args.Path) {
										strncpy(path,cmd_args.Path,sizeof(path)-1);
										path[sizeof(path)-1] = '\0';
									}

									if(cmd_args.Template) {
										strncpy(template,cmd_args.Template,sizeof(template)-1);
										template[sizeof(template)-1] = '\0';
									}
								}
								else
								{
									FillFaultString(db,IoErr(),temp_buffer,sizeof(temp_buffer));
								/*
									ShowEasyRequest(db,Quote(db,MSG_ERROR_READING_ENV_VARIABLE_TXT),
										env_name,temp_buffer);
*/
									error = -1;
								}
...
*/
						...
						else
						{
							FillFaultString(db,IoErr(),temp_buffer,sizeof(temp_buffer));
/*
							ShowEasyRequest(db, "Error reading env-var %s\n%s",
								env_name,temp_buffer);
*/
							DebugPrintF("[ipp.device] Error reading %s. %s\n", env_name, temp_buffer);
							error = -1;
						}

						if(path[0] == '\0') {
							SPrintf(db, path, "T:ippspool%ld", du->du_Index);
							path[sizeof(path)-1] = '\0';
						}

						SPrintf(db, path, "%s%ld", path, GetUniqueID());

						DebugPrintF("[ipp.device] Printer     %s\n", printer);
						DebugPrintF("[ipp.device] Spool file  %s\n", path);
						DebugPrintF("[ipp.device] Template    %s\n", template);
